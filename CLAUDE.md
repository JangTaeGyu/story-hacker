...
> **추리 모드의 단서는 전부 제약 조건이어야 합니다.** 마지막 단서가 정답을 인쇄하면(예전에는 21스테이지 중 14곳이 `"R=255, G=0, B=0 → 255000"` 식이었습니다) 다섯 번 틀리기만 하면 되는 인내심 게임이 됩니다. 지식을 쓰는 편(107·108)도 지식은 출발점으로만 두고, 자릿수 제약으로 답에 닿게 하세요 — 몰라도 추론으로 풀려야 합니다.
...
├── deduction-logic.spec.ts # 추리 단서를 술어로 옮겨 정답·유일성 검산
...
> 추리 모드는 스토리 모드와 채점표가 다릅니다. **필요한 추론 단계 수**로 매깁니다 — 제약을 몇 번 교차시켜야 후보가 하나로 좁혀지는가. 지식이 필요하다는 것 자체는 난이도가 아닙니다(알면 즉답, 모르면 못 풂). 현재 EASY 2편(104·105) · NORMAL 4편(101·102·103·106) · HARD 2편(107·108)입니다.

### 본문과 단서의 역할 분담

**본문(`story`)에 계산식을 쓰지 마세요.** 풀이에 필요한 정보는 전부 단서(`clue`)에만 둡니다. 본문은 인물·긴장·이유를 맡습니다.

한때 EP.11–20의 본문 마지막 문단이 단서를 통째로 낭독했습니다(16/40). 등장인물이 *"멤버 수를 제곱한 값에 프로젝트 코드를 더해. 그걸 도시 코드로 곱하면 끝이야"* 라고 말하고, 바로 아래 단서 블록이 같은 식을 다시 보여주는 식이었습니다. 사이버펑크 껍질을 씌운 연산 문제지에 가까웠고, 무엇보다 같은 정보를 두 번 읽게 했습니다.

지금은 본문이 **시선을 물건으로 넘기는 데서 끝납니다** — "그가 장치를 돌려 보였다. 작은 화면에 세 줄이 떠 있었다." 그다음은 단서 블록이 맡습니다. EP.01–10은 원래 이 방식이었고(수첩 속표지 메모, 긁힌 명판, 카페 포스터), 이제 EP.11–20도 같은 비율입니다(각 1/35, 1/40).
...
단서를 추가·수정하면 **`specs/deduction-logic.spec.ts`의 술어도 함께 고쳐야 합니다.** 그 스펙은 단서 문장을 기계가 검산할 수 있는 술어로 한 번 더 적어 두고, 모든 단서가 정답에 대해 참인지 · 전부 적용하면 후보가 하나로 좁혀지는지를 검사합니다. 실제로 정답만 바꾸고 단서를 그대로 둬서 **여섯 단서 중 셋이 거짓인 스테이지가 배포된 적이 있습니다.** 눈으로는 잘 안 걸립니다.
